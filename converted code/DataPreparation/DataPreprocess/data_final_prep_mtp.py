...
# %%
tbl_output_train = tbl_input_train.withColumn("index", monotonically_increasing_id()).drop("index")
tbl_output_test = tbl_input_test.withColumn("index", monotonically_increasing_id()).drop("index")

# Storing HASHED_CIF for merging after model prediction
id_output_train = tbl_output_train.select("HASHED_CIF")
id_output_test = tbl_output_test.select("HASHED_CIF")

# Features to be dropped if present from dataset at this step
drop_features = ['HASHED_CIF']

# Drop specified features from train and test datasets
tbl_output_train = tbl_output_train.drop(*drop_features)
tbl_output_test = tbl_output_test.drop(*drop_features)
