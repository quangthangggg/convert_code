...
def fun_get_csv_path(dataAssetName, observation_year, observation_month_number, historical_months, 
    ...
    observation_month_number = int(observation_month_number)
    ...
    start_date = datetime(observation_year, observation_month_number, 1) - relativedelta(months=historical_months-1)
    # Next 2 months of data is required for churn flag creation
    end_date = datetime(observation_year, observation_month_number, 1) + relativedelta(months=2)
    if new_data_flag == 'True' and monitoring_flag == 'False':
        ...
        end_date = datetime(observation_year, observation_month_number, 1)
    elif new_data_flag == 'True' and monitoring_flag == 'True':
        # For new data we do not need next 2 months of data (next 2 months of data is required for churn flag creation)
        end_date = datetime(observation_year, observation_month_number, 1) + relativedelta(months=2)
    
    # Generate list of months
    list_of_months = [start_date + relativedelta(months=i) for i in range((end_date.year - start_date.year) * 12 + end_date.month - start_date.month + 1)]
    list_of_months = [month.strftime("%Y_%m") for month in list_of_months]
