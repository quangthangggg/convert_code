...
# %%
tbl_model_pred_data = run.input_datasets['model_pred_data'].to_pandas_dataframe()
...
normalized_macro_recall = (recall_score(true_labels, predicted_labels, average='macro')-0.5)/(1-0.5)
...
ndp_score_data = pd.DataFrame(list(ndp_score_dict.items()), columns=['metrics', 'ndp_score'])
...
orig_model_score_data = pd.DataFrame(list(orig_model_metrics_dict.items()), columns=['metrics', 'original_model_score'])
