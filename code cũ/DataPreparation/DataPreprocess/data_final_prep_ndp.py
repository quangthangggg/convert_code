...
# %%
run = Run.get_context()
tbl_input_new_data = run.input_datasets['new_data'].to_pandas_dataframe()
...
# %%
# Reset index
tbl_output_new_data = tbl_input_new_data.reset_index(drop=True)

# %%
# Storing HASHED_CIF for merging after model prediction
monitoring_flag = args.monitoring_flag
if monitoring_flag == 'True':
    id_output_new_data = tbl_output_new_data[['HASHED_CIF', 'churn_flag']]
else:
    id_output_new_data = tbl_output_new_data[['HASHED_CIF']]

# %%
# Features to be dropped if present from dataset at this step
drop_features = ['HASHED_CIF']

tbl_output_new_data = tbl_output_new_data.drop(columns=drop_features,  errors='ignore')
